    ...
    public class reqDB
    {
        ...
        public void insert_user(string log, string name, string surname, string pass, int id_access)
        {
            MySqlCommand command = new MySqlCommand("INSERT INTO `user` (`id`, `login`, `name`, `surname`, `password`, `id_access`) " +
                "VALUES (NULL, @log, @name, @surname, aes_encrypt(@pas,'potato6'), @id_access)", db_con.getConn());
            command.Parameters.Add("@log", MySqlDbType.VarChar).Value = log;
        ...
        public void insert_history(Int64 cost, string name, string fio, Int64 phone, string email, string address, DateTime date, double discount, int count_prod)
        {
        ...
        public long select_all_count_for_stats(int id)
        {
            DataTable table = new DataTable();
            MySqlCommand command = new MySqlCommand("SELECT SUM(xs + xl + xxl + l + s + m) FROM `size`, `product` WHERE size.id = product.id_size and product.id_storage = " + id + "", db_con.getConn());
            db_con.openConn();
        ...
        public long select_all_cost_for_stats(int id)
        {
            DataTable table = new DataTable();
            db_con.openConn();
            MySqlCommand command = new MySqlCommand("SELECT SUM((xs + xl + xxl + l + s + m)*product.price) FROM `size`, `product` WHERE size.id = product.id_size and product.id_storage = " + id + "", db_con.getConn());
